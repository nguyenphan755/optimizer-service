/**
 * Import sheet DATA từ buffer Excel điện trở (logic đồng bộ scripts/importResistanceExcel.ts).
 */
...
export interface ResistanceImportRunResult {
  inserted: number;
  skipped: number;
  errors: string[];
}

export async function importResistanceFromBuffer(
  client: PoolClient,
  buffer: Buffer,
  options?: { truncate?: boolean }
): Promise<ResistanceImportRunResult> {
  const errors: string[] = [];
  if (options?.truncate) {
  ...
  if (!ws) {
    return {
      inserted: 0,
      skipped: 0,
      errors: ['Không tìm thấy sheet tên "DATA". File điện trở cần sheet DATA (như cadivi_resistance_weight_data).'],
    };
  }

  const rows = XLSX.utils.sheet_to_json<Record<string, unknown>>(ws, { defval: '', raw: false });
  if (rows.length === 0) {
    return { inserted: 0, skipped: 0, errors: ['Sheet DATA không có dòng dữ liệu.'] };
  }
  ...
  if (!colLoai || !colTiet || !colKet || !colNm) {
    return {
      ...
      skipped: 0,
      errors: [
        `Thiếu cột bắt buộc trên sheet DATA. Tìm thấy: loại=${colLoai}, tiết diện=${colTiet}, kết cấu=${colKet}, nhà máy=${colNm}`,
      ],
    };
  }
